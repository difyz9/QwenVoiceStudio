  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
      <div className="surface-panel rounded-[2rem] p-6 lg:p-8">
        ...
        <h2 className="section-title mt-2 text-panel-strong">运行时信息</h2>

        {error ? <div className="mt-6 rounded-[1.35rem] bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</div> : null}
        {isLoading ? <div className="mt-6 rounded-[1.35rem] bg-slate-100 px-4 py-3 text-sm text-slate-600">正在同步系统信息...</div> : null}

        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {items.map((item) => (
            <article key={item.label} className="rounded-[1.7rem] border border-white/45 bg-[rgba(255,252,247,0.88)] p-5 shadow-[0_14px_34px_rgba(77,52,25,0.06)]">
              <div className="section-kicker">{item.label}</div>
              <div className="display-font mt-3 text-3xl font-semibold tracking-tight text-panel-strong">{item.value}</div>
            </article>
          ))}
        </div>
      </div>
      ...
      <div className="surface-panel rounded-[2rem] p-6 lg:p-8">
        <div className="section-kicker">Ops Notes</div>
        <h2 className="section-title mt-2 text-panel-strong">当前可操作项</h2>
        <div className="mt-6 space-y-4 text-sm leading-7 text-slate-600">
          ...
          <div className="rounded-[1.7rem] bg-[linear-gradient(180deg,rgba(239,210,177,0.34),rgba(255,247,236,0.74))] px-5 py-5">
